...
def delete_if_exists(obj: pm.PyNode):
    """
    self explanatory, deletes an object if it exists, continues if it doesn't.
...
class CtrlSet:
    ...
    def create_ctl(self):
        ...
        if self.ctl_shape == "circle":
            ctl_kwargs = {
                "degree": 1,
                "point": self.transform_shape_list(shape_list=self.circle),
            }
        if self.ctl_shape == "sphere":
            ctl_kwargs = {
                "degree": 1,
                "point": self.transform_shape_list(shape_list=self.sphere),
            }
